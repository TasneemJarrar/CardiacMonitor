...
export async function initNurseDashboard() {
  const dashboardContent = document.querySelector("#dashboard-content");
  const role = getRole();
  dashboardContent.innerHTML = `<p class="text-text-muted dark:text-text-muted-dark">Loading Nurse dashboard...</p>`;
...
const [originalPatients, originalAppointments, originalUsers] =
    ...
    const nurse = users.find((user) => user.role === role);
  ...
  <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">
    ...
    document
      ...
      .addEventListener("click", initnurseDashboard);
  }
}
