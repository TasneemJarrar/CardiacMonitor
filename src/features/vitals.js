...
function renderVitals(patients) {
  const VitalsContainer = document.querySelector("#vitals-list");
  const role = getRole();
  ...
  VitalsContainer.innerHTML = patients
    .map((p) => {
      ...
      return `
    <div class="grid grid-cols-12 gap-2 p-2 items-center text-sm border-b border-border last:border-0 dark:border-border-dark hover:bg-background-dark">
