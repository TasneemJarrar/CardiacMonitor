import { fetchData, getItem, getRole } from "../../src/js/storage.js";

export async function initReceptionistDashboard() {
  const dashboardContent = document.querySelector("#dashboard-content");
  const role = getRole();
  dashboardContent.innerHTML = `<p class="text-text-muted dark:text-text-muted-dark">Loading Receptionist dashboard...</p>`;
  ...
  try {
    ...
    let patients = getItem("patients") || originalPatients;
    let appointments = getItem("appointments") || originalAppointments;
    let users = getItem("users") || originalUsers;
    ...
    const receptionist = users.find((user) => user.role === role);
  ...
  <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">
    ...
    <p class="text-xs font-medium mt-1">Upcoming Appointments</p>
        ...
        <div class=" bg-surface dark:bg-surface-dark border border-border dark:border-border-dark rounded-2xl p-5">
          <div class="flex justify-between mb-3">
            ...
            <a href="./appointments.html" class="text-xs text-primary hover:underline transition-all duration-300 ease-in-out">View All</a>
          </div>
          ...
          <div class="flex flex-col gap-2">
            ${upcomingAppointments
              .map((a) => {
                ...
                return `
                <div class="appointment-patient-row flex justify-between items-center w-full p-3 border border-border dark:border-border-dark rounded-xl hover:-translate-y-0.5 hover:border-primary/40 dark:hover:border-primary-light/40 hover:shadow-sm transition-all duration-200 ease-in-out">
                  <div>
    ...
    document
      ...
      .addEventListener("click", initReceptionistDashboard);
  }
}
